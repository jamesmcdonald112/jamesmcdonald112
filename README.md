...
A production-style full-stack demo banking app built with Spring Boot (Java) on the backend and React + TypeScript on the frontend.

#### Backend highlights:
- Account creation with autogenerated card numbers (using BIN + random ID + Luhn algorithm checksum) and PINs.
- Authentication with card number, PIN, and password.
- Deposit and transfer functionality with full validation (balance checks, duplicate prevention, Luhn verification).
- Global exception handling with structured error responses.
- JUnit & Mockito test suite covering services, repositories, and controllers.

#### Frontend highlights:
- React SPA with form validation, toast notifications, and inline error messages.
- Dashboard featuring balance and card views (flip card animation), add-money modal, and transfer modal.
- Strong error handling for API calls with user-friendly feedback.
- Deployed frontend (Netlify) consuming backend API (Render).

##### ⚠️ Note: This project is for learning/demo purposes. Passwords are not hashed, and Render free tier may cause cold starts (up to ~60s).
